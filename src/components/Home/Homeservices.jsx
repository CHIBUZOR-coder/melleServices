      ...
      >
        ...
        <ul className="space-y-1.5 mb-4 flex-1">
          {service.highlights.slice(0, 3).map((h) => {
            const [label] = h.includes(": ") ? h.split(": ") : [h];
            return (
              <li key={h} className="flex items-center gap-2 text-[11px] text-gray-400 uppercase tracking-wider font-semibold">
                <span className="w-1 h-1 rounded-full bg-[#e87722]" />
                {label}
              </li>
            );
          })}
